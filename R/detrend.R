rlm <- function(
    x, y, weights, ..., w = rep(1, nrow(x)), init = "ls",
  ...
  irls.rrxwr <- function(x, w, r) {
    w <- sqrt(w)
    max(abs((matrix(r * w, 1L, length(r)) %*% x)/sqrt(matrix(w, 1L, length(r)) %*% (x^2))))/sqrt(sum(w * r^2))
  }
  ...
  if (qr(x)$rank < ncol(x)) stop("'x' is singular: singular fits are not implemented in 'rlm'")
  if (!(any(test.vec == c("resid", "coef", "w", "NULL")) || is.null(test.vec))) stop("invalid 'test.vec'")

  xx <- x
  yy <- y

  if (!missing(weights)) {
    if (length(weights) != nrow(x)) stop("length of 'weights' must equal number of observations")
  ...
  if (method == "M") {
    ...
    scale.est <- "MM"
    temp <- do.call("lqs", c(list(x, y, intercept = FALSE, method = "S", k0 = 1.548), lqs.control))
    coef <- temp$coefficients
    resid <- temp$residuals
    psi <- psi.bisquare
  ...
  if (!done) warning(gettextf("'rlm' failed to converge in %d steps", maxit), domain = NA)
  fitted <- drop(xx %*% coef)
  cl <- match.call()
  cl[[1L]] <- as.name("rlm")
...
detrend <- function(x) {
  X <- matrix(c(rep(1, NROW(x)), as.numeric(index(x))), ncol = 2L)
  residuals(MASS::rlm(
    X, as.numeric(x), method = "MM", scale.est = "Huber", na.action = na.exclude
  ))
}
